...
|---|---|---|---|
| 2026-09-13 (session 6, C2 tool bug) | §0.2 C2 / instrument | **The annotation tool dropped every SHARE keypress in the message task.** `scripts/annotate.py` bound skip to the key `s`, which is the message codebook's SHARE code, so pressing `s` silently skipped the item. The coder (JAG) reported by email she used `s`=SHARE and skipped nothing; the data shows 156/200 recorded, all `u`, and the 44 missing items are scattered across positions 0-194 with the coder reaching the final item — consistent with dropped `s` presses, not an early stop. | Tool bug, surfaced by the coder. **C2's conclusion is unaffected**: SHARE and UNCL are both non-objection and no objection key (o/r/e/w) was pressed, so objections = 0 over both the 156 recorded and, per the coder, the 44 dropped (all SHARE); the coded denominator is 200, all non-objection, which tightens the one-sided bound to ~0.013. The SHARE-vs-UNCL distribution is unusable (SHARE dropped, possibly rerouted to `u`), but C2 only ever reported the zero-objection bound. Tool fixed (skip=Enter, quit=`.`, reserved keys asserted against every codebook) with a regression test; the shipped bundle had the bug. |
| 2026-09-13 (session 5, monitor experiment) | §10 non-goals; new analysis | **Ran the AgentHarm harmful `test_public` split** on claude-haiku-4.5 (both interleaved configs) to get ground-truth harmful trajectories for a monitor-recall experiment: two gpt-5-nano monitors, one reading only reasoning and one only actions, scored per step. | Departs from 'no offensive cyber evaluations / benign by construction'. Authorised by the author 2026-09-13. Tools are AgentHarm's synthetic mocks (nothing executes); ground truth is AgentHarm's own published grader; only counts, rates and monitor verdicts are stored, never task, reasoning or action text. Exploratory. The run was underpowered (48/50 refused per arm, 2 positives each), which is reported as the result. |
